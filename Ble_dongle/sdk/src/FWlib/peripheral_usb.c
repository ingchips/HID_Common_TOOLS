...
USB_ERROR_TYPE_E USB_SendData(uint8_t ep, void* buffer, uint16_t size, uint32_t flag)
{
    ...
    uint32_t prim = NVIC_GetEnableIRQ(platform_read_info(PLATFOFM_INFO_IRQ_NUMBER + PLATFORM_CB_IRQ_USB));
    platform_enable_irq(PLATFORM_CB_IRQ_USB, 0);
    USB_StartTransfer(USB_EP_DIRECTION_IN(ep), buffer, size, flag);
    if (prim)
        platform_enable_irq(PLATFORM_CB_IRQ_USB, 1);
