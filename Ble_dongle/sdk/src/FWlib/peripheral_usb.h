...
typedef struct __attribute__((packed))
{
    ...
    uint8_t  nbInterface; 		//?м??????
    uint8_t  configIndex; 		//?????????????
    uint8_t  iDescription;		//???????????
    uint8_t  attrib;      		//bit:7 = 1, bit:6 = 0;????USB??????? bit:5 ?????????????
    uint8_t  maxPower;    		//?????????????
...
typedef struct __attribute__((packed))
{
    ...
    uint8_t  nbEp;            		//????????????????
    uint8_t  usbClass;        		//??????????????USBЭ??汾
    uint8_t  usbSubClass;     		//??????BIOS
    uint8_t  usbProto;        		//???Э???0X02 ????Э???0X01
    uint8_t  iDescription;    		//??????????????
} USB_INTERFACE_DESCRIPTOR_REAL_T;
// =============================================================================
/// HID Class descriptor structure HID ????????
// =============================================================================
typedef struct __attribute__((packed))
{
	uint8_t		size;        	//HID ?????????? 0x09
	uint8_t		type;        	//HID ?????????? 0X21
	uint16_t	bcd;         	//HID ?汾 ???????
	uint8_t		countryCode; 	//?豸?????????
	uint8_t		nbDescriptor;	//?????????????
	uint8_t		classType0;  	//????????????? 0X22?????????? 0X23??????????
	uint16_t	classlength0;	//?????????????? ??λ???
...
typedef struct __attribute__((packed))
{
    uint8_t  size;      		//????????????? 0x07
    uint8_t  type;      		//????????????? 0X05
    uint8_t  ep;        		//bit8 1:IN 0:OUT ??????USB???
    uint8_t  attributes;		//????? ????????ж????????????
    uint16_t mps;       		//????????????
    uint8_t  interval;  		//??????
