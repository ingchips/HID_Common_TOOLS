...
static void IAP_JumpUsbBoot_Delay_Timeout_Callback(void){
    IAP_APP_ERROR("[CMD] jump usb boot.\n");
    app_main_set_jump_boot_flag();
}
...
static IAP_APP_ErrCode_t IAP_CMD_Reboot_handler(uint8_t * payload, uint16_t length){
	...
	platform_set_timer(IAP_Reboot_Delay_Timeout_Callback, *delay_ms * 1000 / 625);
    return errCode;
}
...
static IAP_APP_ErrCode_t IAP_CMD_SwitchApp_handler(uint8_t * payload, uint16_t length){
	...
	platform_set_timer(IAP_DoUpdate_Delay_Timeout_Callback, *delay_ms * 1000 / 625);

    return errCode;
}

static IAP_APP_ErrCode_t IAP_CMD_SwitchBoot_handler(uint8_t * payload, uint16_t length){
	...
	platform_set_timer(IAP_JumpUsbBoot_Delay_Timeout_Callback, *delay_ms * 1000 / 625);
...
static IAP_APP_ErrCode_t IAP_APP_cmd_dispatch(uint8_t *data, uint16_t len){
    ...
    switch(APP_CMD->CMD){
        ...
        case IAP_CMD_SWITCH_BOOT:{
            ...
            errCode = IAP_CMD_SwitchBoot_handler(APP_CMD->payload, APP_CMD->length);
        }break;
